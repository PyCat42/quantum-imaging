...
- [src](src):
  ...
  - [imaging_systems.py](src/imaging_systems.py):
  - [object_funcs.py](src/object_funcs.py): Implements mathematical representation of objects for imaging in the form:
    ```python
    def object_func(x, y):
        return transmission, phase
    ```
  - [resolution_estimation.py](src/resolution_estimation.py):
  - [MC_helpers](src/MC_helpers.py):
  - [sellmeier.py](src/sellmeier.py):
  - [ray_propagation](src/ray_propagation.py):
  - [plotting_helpers](src/plotting_helpers.py):
- [tests](tests):
  - [SPDC_tests.ipynb](tests/SPDC_tests.ipynb):
  - [imaging_test.ipynb](tests/imaging_test.ipynb):
  - [resolution_test.ipynb](tests/resolution_test.ipynb):
...
$$\omega_p = \omega_s + \omega_i \qquad \mathbf{k}_p = \mathbf{k}_s + \mathbf{k}_i + \mathbf{k}_m,$$
...
$$\Delta k_x = k_{p,x} - k_{s,x} - k_{i,x}, \qquad \Delta k_y = k_{p,y} - k_{s,y} - k_{i,y},$$
...
$$E(\mathbf{r}) = E_0 \exp\left[-\frac{x^2+y^2}{\omega_0^2}\right] e^{-ik_p z} e^{i\omega_p t}.$$
...
The interaction time represents the time during which the pump field interacts with the crystal and 
is calculated from the pump group velocity in the crystal:

$$T_I = \frac{L}{v_g} = \frac{L n_g}{c} = \frac{L}{c}\left(n_p - \lambda_p \frac{dn_p}{d\lambda_p}\right).$$
...
The multidimensional transition-rate integral is evaluated using the QMC method:
...
### SPDC spectrum simulation

Using this simulation we can vary SPDC source parameters and see how 
downconverted photon spectrum changes. Some examples are given bellow.
<table>
  <tr>
    <td><img src="images/spectrum_L0.001_t25_w1e-05_ispravljeno.png" alt="First Figure" width="100%"></td>
    <td><img src="images/spectrum_L0.001_t25_w0.0001_ispravljeno.png" alt="Second Figure" width="100%"></td>
    <td><img src="images/spectrum_L0.001_t25_w0.00035_ispravljeno.png" alt="Second Figure" width="100%"></td>
  </tr>
</table>
...
This is done since not all parts of the spectrum need an equal amount of evaluations.
...
optimization of an interferometric setup in Michelson $\mathrm{SU}(1,1)$
...
are then combined in the detector arm, and information about the
object can be obtained from their interference.
![Setup Scheme](images/michaelson_setup.png)
...
$$V(x,y) = \frac{\Gamma^+(x,y)-\Gamma^-(x,y)} {\Gamma^+(x,y)+\Gamma^-(x,y)}.$$
...
The source class must have attributes like pump wavelength, central signal wavelength, etc.,
and a method for sampling source photons,
but in general, the imaging simulation is adaptable to sources other than SPDC.

### Propagation through the Setup

Created samples are then propagated through the setup using the transfer matrix method.
...
To do this, a specialized class `ImagingSystem` is created.
...
![Full simulation example. Object is 5mm wide 100% absorbing bar](images/full_sim_output_example.png)
...
(which happens in the paraxial approximation),
the object-plane visibility can be approximated by convolution with a normalized imaging kernel:

$$V(\rho_D) = t_i \int \hat{A}(k_s - k_i) t_o\bigl(\rho_O(k_i)\bigr) \, dk_i.$$
...
![System Kernel](images/psf_analysis.png)

Convolution of this kernel with the object function gives us object-plane visibility,
which then needs to be scaled using system magnification factor to obtain detector visibility.

### Setup Resolution Analysis

Resolution is evaluated using two absorbing bars with a variable gap.
![Resolution target](images/res_target.png)
...
$$R = \frac{V_{\mathrm{middle}} - V_{\mathrm{bar}}} {V_{\mathrm{background}} - V_{\mathrm{bar}}}.$$

The pair is treated as resolved when $R \ge 0.735$.

To determine the resolution, one can first obtain the FWHM of the system PSF, which should give a good guess of the value.
Then a scan can be performed using the fast simulation method
to calculate visibility for varying gap sizes around that value.
The last gap value that satisfies the criterion is the setup resolution. 

<table>
  <tr>
    <td><img src="images/visibility_at_limit.png" alt="First Figure" width="100%"></td>
    <td><img src="images/visibility_at_limit_profile.png" alt="Second Figure" width="100%"></td>
  </tr>
</table>

To confirm this result precisely, one could run a full simulation on a few selected gap values,
...
its own resolution stands in the way of determining setup resolution).
